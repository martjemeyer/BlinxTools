...
public class BitBlinxMainAdapter extends RecyclerView.Adapter<BitBlinxMainAdapter.MainHolder> {
    ...
    class MainHolder extends RecyclerView.ViewHolder {
        ...
        void setDetails(Result result) {
            ...
            String rightPair = result.symbol.substring(result.symbol.lastIndexOf("/") + 1);
            boolean btcPair = rightPair.contains("BTC");

            boolean negativeChange = result.priceChange.contains("-");
            int gainColor = ContextCompat.getColor(mContext, negativeChange ? R.color.red : R.color.green);
            ...
            lastPriceDetailsBtc.setText(removeTrailingZeros(Float.parseFloat(result.last)) + " " + (btcPair ? "BTC" : rightPair));
            ...
            if (btcPair && btcValuePreferredCurrency != -1){
                ...
                float valueInCurrency = Float.parseFloat(result.last) * btcValuePreferredCurrency;
                String valueInCurrencyString = removeTrailingZeros(valueInCurrency);
        ...
        private String removeTrailingZeros(float input) {
            BigDecimal stripedVal = new BigDecimal(String.valueOf(input)).stripTrailingZeros();
